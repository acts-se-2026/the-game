...
export default function LoadingPage() {
  ...
  const enterLobby = (event: FormEvent<HTMLFormElement>) => {
    ...
    navigate('/', { state: { username: cleanUsername } })
  }
  ...
  return (
    ...
    >
      <form onSubmit={enterLobby} className="max-w-lg rounded-2xl border border-slate-800 bg-slate-900/85 p-6 shadow-2xl shadow-black/20">
        <label htmlFor="username" className="block text-sm font-black text-white">Username</label>
        <input
          id="username"
          value={username}
          onChange={(event) => {
        ...
        />
        {error && <p role="alert" className="mt-2 text-sm font-bold text-rose-300">{error}</p>}
        <button type="submit" className="mt-5 w-full rounded-xl bg-blue-500 px-5 py-3 font-black text-white transition hover:bg-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-300">
          Enter lobby
        </button>
      </form>
    </PageShell>
